...
public class VerbCauseEntailmentRelations {
    public static void main (String[] args) {
        Iterator<ISynset> verbIter = WordNetUtils.getSynsetIterator(POS.VERB);
        ...
        while (verbIter.hasNext()) {
            ...
            if (causeIds != null && !causeIds.isEmpty()) {
                List<ISynset> synsets = causeIds.stream().map(WordNetUtils::getSynsetBySynsetId).collect(Collectors.toList());
            ...
            if (entailmentIds != null && !entailmentIds.isEmpty()) {
                List<ISynset> synsets = entailmentIds.stream().map(WordNetUtils::getSynsetBySynsetId).collect(Collectors.toList());
    ...
    private static void writeToFile(String path, Map<ISynset, List<ISynset>> map) {
        try {
            ...
            for (Map.Entry<ISynset, List<ISynset>> entry : map.entrySet()) {
                String line = WordNetUtils.synset2String(entry.getKey(), true).concat("\tCAUSES\t");
                List<String> synsetStr = entry.getValue().stream().map(synset -> WordNetUtils.synset2String(synset, true)).collect(Collectors.toList());
